...
class FeatherPDF(QMainWindow):
    ...
    def wheelEvent(self, event: QWheelEvent):
        ...
        if num_steps > 0:
            self.prevPage()
            self.pdf_view.verticalScrollBar().setValue(self.pdf_view.verticalScrollBar().minimum())
        else:
            self.nextPage()
            self.pdf_view.verticalScrollBar().setValue(self.pdf_view.verticalScrollBar().maximum())
